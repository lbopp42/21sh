...
/*   Updated: 2017/05/14 12:06:16 by lbopp            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple	*iscmd_prefix(t_token *tok_lst, int nb_tok, int mv)
{
	...
	if ((tuple_parse = isio_redirect(tok_lst, nb_tok, 0)) || (tuple_parse = is_word(tok_lst, 0)))
	{
				...
				(tuple_parse = is_word(tok_lst, max_tuple->mv)))
		{
			...
			else if (!max_tuple->ast_tree->left)
			{
...
void	print_AST(t_ast_node *ast_tree, int mode)
{
	if (!ast_tree)
		return ;
	if (ast_tree->left)
		print_AST(ast_tree->left, 1);
	if (ast_tree->right)
		print_AST(ast_tree->right, 2);
	if (mode == 1)
		printf("LEFT = [%s]\n", ast_tree->content);
	else if (mode == 2)
		printf("RIGHT = [%s]\n", ast_tree->content);
...
int	main(void)
{
	...
	tok_lst->next->type = WORD;
	tok_lst->next->content = ft_strdup("test");
	...
	tok_lst->next->next->next->next->next = NULL;
	tuple_end = is_simplecmd(tok_lst, 3, 0);
	if (tuple_end)
	{
		printf("mv final = %d\n", tuple_end->mv);
		print_AST(tuple_end->ast_tree, 0);
	}
}
