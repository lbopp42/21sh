...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/05/18 10:14:56 by lbopp             #+#    #+#             */
/*   Updated: 2017/05/18 10:21:50 by lbopp            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		**array;
	const char	*array_ref[4] = {"ls -a -l", "cat -e", "|", NULL};
	int			i;
...
void	test_parser15(t_token *tok_lst, int nb_tok)
{
	t_tuple	*tmp;

	tmp = iscomplete_cmd(tok_lst, nb_tok, 0);
	if (nb_tok == tmp->mv && is_valid_ast(tmp->ast_tree))
		printf("\033[32m[OK]   \033[0m[ls -a -l|cat -e]\n");
	else
	{
		printf("\033[31m[FAIL] \033[0m[ls -a -l|cat -e]\n");
	}
}
