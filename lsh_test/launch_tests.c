...
/*   Updated: 2017/05/18 10:23:08 by lbopp            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	launch_test_lexer_from_1_to_25(t_token **array, int *i, int	**nb_tok)
{
	...
	*i += 1;
	array[*i] = test_lexer14(nb_tok, *i);
	*i += 1;
	array[*i] = test_lexer15(nb_tok, *i);
	*i += 1;
	array[*i] = test_lexer16(nb_tok, *i);
}
...
void	launch_test_parser_from_1_to_25(t_token **array, int *nb_tok)
{
	...
	i += 1;
	test_parser14(array[i], nb_tok[i]);
	i += 1;
	test_parser15(array[i], nb_tok[i]);
	i += 1;
	test_parser16(array[i], nb_tok[i]);
	/*while (array[i])
	{
